...
export default class Project extends BidEntity {
  ...
  set projectStatus(val) {
    if (val.id && val.core_status) {
      const oldValue = cloneDeep(this._data.project_status);
  ...
  assess() {
    ...
    Object.values(this.bids).forEach(bid => {
      if (bid.isActive) {
  ...
  _calculateMargin() {
    ...
    return Helpers.confirmNumber(round(margin, 4));
  }
  ...
  _clearPortfolio() {
    this._propertiesToSum.forEach(prop => {
      this._data[prop] = 0;
    });
  }
