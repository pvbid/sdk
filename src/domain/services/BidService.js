...
import DynamicGroupScaffolding from "../scaffolding/DynamicGroupScaffolding";
...
export default class BidService {
  ...
  async addMetric(bid, title) {
    if (this.context.user.can("edit-bid")) {
   ...
   */
  async addDynamicGroup(bid, title) {
    if (this.context.user.can("edit-bid")) {
      const scaffolding = DynamicGroupScaffolding.create(bid.id, title);
      const obj = await this.repositories.dynamicGroups.create(bid.id, scaffolding);
      const dynamicGroup = new DynamicGroup(obj, bid);
      bid._data.dynamic_groups[dynamicGroup.id] = dynamicGroup;
      dynamicGroup.bind();
      dynamicGroup.on("assessed", "dynamic_group." + dynamicGroup.id, () => {
        waitForFinalEvent(() => bid.assess(), 15, `bid.${bid.id}.dynamic_group`);
      });
      dynamicGroup.on("assessed", `bid.${bid.id}`, () => bid._handleAssessmentCompleteEvent());
      return dynamicGroup;
    } else throw new Error("User does not have permission to add Dynamic Group.");
  }
